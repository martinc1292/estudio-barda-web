...
function Isotipo({ size = 32, variant = "color", title = "Barda Arquitectura — isotipo" }) {
  ...
  //   └──────────────────────┘
  // viewBox 26 × 18 — dos planos desplazados con espacio entre ambos (manual 02.2 / 05.1)
  const w = size;
  const h = Math.round((size * 18) / 26);
  return (
    <svg width={w} height={h} viewBox="0 0 26 18" xmlns="http://www.w3.org/2000/svg"
...
function LogoHorizontal({ height = 40, variant = "color", inkColor }) {
  ...
  // Isotipo: ratio 26:18. Su alto debe acompañar la caja del logotipo.
  const isoSize = Math.round(height * 26 / 16);
  const gap = Math.round(height * 0.34);
  return (
    <div className="bd-logo bd-logo-h" style={{ display: "flex", alignItems: "center", gap }}>
...
function LogoVertical({ height = 64, variant = "color", inkColor }) {
  const ink = inkColor
    || (variant === "mono-light" || variant === "mono-white" ? "#F8F0E5" : "#0F0F10");
  const isoSize = Math.round(height * 1.15);
  return (
    <div className="bd-logo bd-logo-v" style={{
      display: "inline-flex", flexDirection: "column",
      alignItems: "center", gap: height * 0.16,
    }}>
      <Isotipo size={isoSize} variant={variant} />
      <div style={{
        fontFamily: "var(--sans)",
...
function TopBar({ route, setRoute, t, lang, theme }) {
  // Logo oficial según theme: negro sobre fondo claro (Arena), ladrillo sobre fondos oscuros (Piedra/Hierro)
  const logoSrc = theme === "light"
    ? "assets/logo-v-negro.png"
    : "assets/logo-v-ladrillo.png";
  return (
    <header className="topbar">
      <a className="brand" onClick={()=>setRoute("index")} style={{cursor:"pointer"}}>
        <img src={logoSrc} alt="Barda Arquitectura" className="brand-logo" />
      </a>
      <div></div>
      <nav className="nav">
...
function App() {
  ...
  return (
    <div className="app">
      ...
      <TopBar route={route} setRoute={setRoute} lang={lang} theme={theme}/>
